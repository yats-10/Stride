...
`getSiteStructuredData()` — builds the `Person` + `CreativeWork` + `WebSite` JSON-LD
graph rendered by the root layout. **Not `Organization`** — Stride is a fictional
brand ([[decisions-log]] ADR-0017). See [[seo-metadata]].
