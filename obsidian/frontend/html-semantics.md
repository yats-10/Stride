...
- In-page anchor targets get a stable `id` (also powers hash-link scrolling —
  see [[smooth-scroll]]). **No placeholder `href="#"`** — a link either goes
  somewhere real or is not a link. Labels and their destinations travel together as
  a `SiteLink` from content; see [[routing]] (In-page navigation).
