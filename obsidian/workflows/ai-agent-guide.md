...
> [!tip] Where to start
> The home view (`src/views/home.tsx`, route `/`) is **complete** — Stride is a
> finished single-page editorial concept. Read the section's view under
> `src/views/` and its content in `src/data/mocks/` before changing anything; follow
> [[new-page]] only when adding a genuinely new section.

> [!warning] Stride is a fictional brand
> It is a published concept piece credited to Yatharth Madaan. Don't add invented
> contact details, social accounts, `@handles`, or `Organization` schema, and never
> ship a placeholder `href="#"`. See [[decisions-log]] ADR-0017.
